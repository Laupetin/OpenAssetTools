...
namespace
{
    TEST_CASE("RawLoaderPhysPreset(T5): Returns no action when file is missing", "[t5][physpreset][assetloader]")
    {
        ...
        auto loader = phys_preset::CreateRawLoaderT5(memory, searchPath, zone);
        REQUIRE_FALSE(loader->CreateAsset("brick", context).HasBeenSuccessful());
    }

    TEST_CASE("RawLoaderPhysPreset(T5): Can parse physpreset", "[t5][physpreset][assetloader]")
    {
        MockSearchPath searchPath;
    ...
    TEST_CASE("RawLoaderPhysPreset(T5): Clamps and scales mass", "[t5][physpreset][assetloader]")
    {
        MockSearchPath searchPath;
        Zone zone("MockZone", 0, GameId::T5, GamePlatform::PC);
        MemoryManager memory;
    ...
    TEST_CASE("RawLoaderPhysPreset(T5): Sets friction to MAX_FRICTION when isFrictionInfinity is set", "[t5][physpreset][assetloader]")
    {
        MockSearchPath searchPath;
        searchPath.AddFileData("physic/brick",
