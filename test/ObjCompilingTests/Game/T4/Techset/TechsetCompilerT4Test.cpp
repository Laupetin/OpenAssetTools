...
TEST_CASE("TechsetCompilerT4", "[techset][t4][compiler]")
{
    ...
    const auto sut = techset::CreateTechsetCompilerT4(memory, searchPath);
...
"lit spot":
  example_lit_spot;
        ...
        // T4 sets loadForRenderer for both sm2 and sm3 techsets.
...
"lit instanced omni shadow":
  example_lit_spot;
        ...
        CHECK(techset->techniques[TECHNIQUE_LIT_FADE_CHARRED] == exampleLitOmni);
        CHECK(techset->techniques[TECHNIQUE_LIT_OMNI_SHADOW_FADE_CHARRED] == exampleLitOmni);
        CHECK(techset->techniques[TECHNIQUE_LIGHT_SPOT_SHADOW_CHARRED] == exampleLitOmni);
