#include "CSVFromStructConverter.h"

#include <cassert>

using namespace IW3;

void CSVFromStructConverter::FillFromBaseField(const cspField_t& field)
{
...
void CSVFromStructConverter::FillInfoString()
{
...
CSVFromStructConverter::CSVFromStructConverter(const void* structure, const cspField_t* fields, const size_t fieldCount)
    : CSVFromStructConverterBase(structure),
      m_fields(fields),
      m_field_count(fieldCount)
{
}

CSVFromStructConverter::CSVFromStructConverter(const void* structure,
                                                             const cspField_t* fields,
                                                             const size_t fieldCount,
                                                             std::function<std::string(scr_string_t)> scriptStringValueCallback)
    : CSVFromStructConverterBase(structure, std::move(scriptStringValueCallback)),
      m_fields(fields),
      m_field_count(fieldCount)
{
}
