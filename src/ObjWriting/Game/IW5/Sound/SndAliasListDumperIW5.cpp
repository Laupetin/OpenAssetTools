...
namespace
{
    ...
    void WriteAliasToFile(CsvOutputStream& stream, const snd_alias_t& alias, int sequence = 0)
    {
        ...
        // vol_mod_index
        WriteColumnIntegral(stream, alias.volModIndex);
        ...
        if (alias.mixerGroup)
        {
            WriteColumnString(stream, alias.mixerGroup);
        }
        else
        {
            WriteColumnString(stream, "");
...
{
    void DumperIW5::DumpAsset(AssetDumpingContext& context, const XAssetInfo<AssetSound::Type>& asset)
    {
