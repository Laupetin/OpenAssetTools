...
namespace
{
    ...
    // Build editorFlags string from element fields
                       ...
                       const FxElemVelStateSample* samples, uint32_t count)
    {
        ...
        {
            float shifted = std::abs(samples[i].local.velocity.base.v[axis] * static_cast<float>((count - 1) * 1000));
            if (maxValue < shifted) maxValue = shifted;

            shifted = std::abs((samples[i].local.velocity.base.v[axis] + samples[i].local.velocity.amplitude.v[axis]) * static_cast<float>((count - 1) * 1000));
            if (maxValue < shifted) maxValue = shifted;
        }
        ...
        for (uint32_t i = 0; i < count; i++)
        {
            ...
            float curvePoint = (maxValue > 0) ? (samples[i].local.velocity.base.v[axis] * static_cast<float>((count - 1) * 1000) / maxValue / 2.0f) : 0.0f;
        ...
        {
            float timeFrame = (i > 0) ? (static_cast<float>(i) / static_cast<float>(count - 1)) : 0.0f;
            float curvePoint = (maxValue > 0) ? (samples[i].local.velocity.amplitude.v[axis] * static_cast<float>((count - 1) * 1000) / maxValue / 2.0f) : 0.0f;
            s << std::format("\t\t\t{} {} 0 0\n", timeFrame, curvePoint);
        }
        s << "\t\t}\n";
    ...
    void WriteVisSamples(std::ostream& s, const FxElemVisStateSample* samples, uint32_t count)
    {
        ...
        if (visSamples.size() == 1)
        {
            ...
            defaultSample.base.color[0] = -1; // 255 unsigned
    ...
    void DumpElement(std::ostream& s, const FxElemDef& elem, uint32_t elemIndex, uint32_t elemType, bool looping)
    {
        ...
        // Spawn ranges (inverse: OAT stores as positive, .efx expects negative)
        ...
        // Spawn angles (radians to degrees)
        for (int i = 0; i < 3; i++)
        {
        ...
        // Default world velocity graphs (empty)
        {
        ...
        // Collision
        float colRadix = (std::abs(elem.collMins.v[0]) + std::abs(elem.collMaxs.v[1])) / 2.0f;
        s << std::format("\tcollOffset {} {} {};\n",
            elem.collMaxs.v[0] - colRadix, elem.collMaxs.v[1] - colRadix, elem.collMaxs.v[2] - colRadix);
        ...
        // Trail basic params
        const auto* trailDef = elem.extended.trailDef;
        if (trailDef)
        {
        ...
        // Trail definition (vertices + indices)
        if (trailDef)
        {
            if (trailDef->vertCount > 0 && trailDef->verts)
            {
                ...
                {
                    s << std::format("\t\t {} {} {}\n",
                        trailDef->verts[v].pos.v[0], trailDef->verts[v].pos.v[1], trailDef->verts[v].texCoord);
                }
                s << "\t} {\n";
            }
        ...
        // Billboard pivot
        s << std::format("\tbillboardPivot {} {};\n", elem.billboardPivot.v[0] / 2.0f, elem.billboardPivot.v[1] / -2.0f);
