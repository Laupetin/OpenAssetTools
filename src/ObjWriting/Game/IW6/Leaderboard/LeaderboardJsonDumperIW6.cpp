#include "LeaderboardJsonDumperIW6.h"

#include "Game/IW6/CommonIW6.h"
#include "Game/IW6/Leaderboard/JsonLeaderboardDef.h"
...
namespace
{
    ...
    private:
        ...
        static void CreateJsonLeaderboardDef(JsonLeaderboardDef& jLeaderboardDef, const LeaderboardDef& leaderboardDef)
        {
            ...
            for (auto i = 0; i < LBTRK_TYPE_COUNT; ++i)
            {
                const auto trackTypeMask = 1 << i;

                if (leaderboardDef.trackTypes & trackTypeMask)
                    jLeaderboardDef.trackTypes.emplace_back(static_cast<LbTrackTypes>(i));
            }
        }
...
{
    void JsonDumperIW6::DumpAsset(AssetDumpingContext& context, const XAssetInfo<AssetLeaderboard::Type>& asset)
    {
