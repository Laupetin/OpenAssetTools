#include "WeaponDumperIW6.h"

#include "Game/IW6/CommonIW6.h"
#include "Game/IW6/InfoString/InfoStringFromStructConverter.h"
...
namespace
{
    ...
    {
    public:
        InfoStringFromWeaponConverter(const WeaponCompleteDef* structure,
    ...
    protected:
        void FillFromExtensionField(const cspField_t& field) override
        {
            ...
            {
                const auto* perSurfaceTypeSound = *reinterpret_cast<snd_alias_list_t**>(reinterpret_cast<uintptr_t>(m_structure) + field.iOffset);

                if (perSurfaceTypeSound && perSurfaceTypeSound->aliasName)
                {
                    const std::string firstBounceSound(perSurfaceTypeSound->aliasName);
            ...
            {
                const auto* keys = reinterpret_cast<scr_string_t*>(reinterpret_cast<uintptr_t>(m_structure) + field.iOffset);
                const auto* values = &keys[std::extent_v<decltype(WeaponDef::notetrackRumbleMapKeys)>];
                std::stringstream ss;
                bool first = true;

                for (auto i = 0u; i < std::extent_v<decltype(WeaponDef::notetrackRumbleMapKeys)>; i++)
                {
                    const auto& key = m_get_scr_string(keys[i]);
                    const auto& value = m_get_scr_string(values[i]);
                    if (!key.empty())
    ...
    private:
        ...
        void FillFromAnimOverrides(const std::string& key)
        {
            ...
            for (auto i = 0u; i < m_weapon->numAnimOverrides; i++)
            {
                const auto& animOverride = m_weapon->animOverrides[i];
        ...
        void FillFromSoundOverrides(const std::string& key)
        {
            ...
            for (auto i = 0u; i < m_weapon->numSoundOverrides; i++)
            {
                const auto& soundOverride = m_weapon->soundOverrides[i];
        ...
        void FillFromFxOverrides(const std::string& key)
        {
            ...
            for (auto i = 0u; i < m_weapon->numFXOverrides; i++)
            {
                const auto& fxOverride = m_weapon->fxOverrides[i];
        ...
        void FillFromReloadOverrides(const std::string& key)
        {
            ...
            for (auto i = 0u; i < m_weapon->numReloadStateTimerOverrides; i++)
            {
                const auto& reloadOverride = m_weapon->reloadOverrides[i];
        ...
        void FillFromNoteTrackOverrides(const std::string& key)
        {
            ...
            for (auto i = 0u; i < m_weapon->numNotetrackOverrides; i++)
            {
        ...
        const WeaponCompleteDef* m_weapon;
    };
    ...
    void CopyToCompleteDef(const WeaponDef* weapon, WeaponCompleteDef* completeDef)
    {
        /*completeDef->weapDef = *weapon;

        if (completeDef->weapDef)
        {
            completeDef->weapDef = *weapon;
    ...
    {
        const auto completeDef = std::make_unique<WeaponCompleteDef>();
        memset(completeDef.get(), 0, sizeof(completeDef));
        /*CopyToCompleteDef(asset.Asset(), completeDef.get());
...
namespace weapon
{
    void DumperIW6::DumpAsset(AssetDumpingContext& context, const XAssetInfo<AssetWeapon::Type>& asset)
    {
