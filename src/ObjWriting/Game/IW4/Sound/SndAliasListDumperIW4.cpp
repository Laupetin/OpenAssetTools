...
namespace
{
    ...
    void WriteAliasToFile(CsvOutputStream& stream, const snd_alias_t& alias, int sequence = 0)
    {
        ...
        // velocity_min
        // TODO: Find min in IDA
        WriteColumnFloat(stream, alias.velocityMin, 0.0f);
        ...
        // mixerGroup
        // TODO: See if this actually needs dumped
        if (alias.mixerGroup)
        {
            WriteColumnString(stream, FromReferencedString(alias.mixerGroup));
        }
        else
        {
            WriteColumnString(stream, "");
        ...
        // TODO: These IW3 flags are likely wrong (maybe not)
        // reverb
...
{
    void DumperIW4::DumpAsset(AssetDumpingContext& context, const XAssetInfo<AssetSound::Type>& asset)
    {
