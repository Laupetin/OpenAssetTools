...
namespace
{
    ...
    private:
            ...
            for (auto i = 0; i < TECHNIQUE_COUNT; i++)
            {
                ...
                if (!techniqueName.empty())
                {
                    ...
                    const auto foundStateBits = std::ranges::find_if(m_state_bits,
                                                                     ...
                                                                     {
                                                                         return s1.loadBits.raw[0] == stateBitsForTechnique.loadBits.raw[0]
                                                                                && s1.loadBits.raw[1] == stateBitsForTechnique.loadBits.raw[1];
                                                                     });
        ...
        GfxStateBits CalculateStateBitsWithStateMap(const state_map::StateMapDefinition* stateMap) const
        {
            ...
            stateMapHandler.ApplyStateMap(m_base_state_bits.loadBits.raw, outBits.loadBits.raw);
        ...
        void AddMapTexture(const std::string& typeName,
                           const TileMode_e tileMode,
                           const GdtFilter_e filterMode,
                           ...
                           const std::string& textureName)
        {
            ...
            textureDef.samplerState.mipMap = 0;
            textureDef.semantic = static_cast<unsigned char>(semantic);

            switch (tileMode)
            {
            case TileMode_e::TILE_BOTH:
                textureDef.samplerState.clampU = 1;
                textureDef.samplerState.clampV = 1;
                textureDef.samplerState.clampW = 1;
                break;
            case TileMode_e::TILE_HORIZONTAL:
                textureDef.samplerState.clampU = 0;
                textureDef.samplerState.clampV = 1;
                textureDef.samplerState.clampW = 0;
                break;
            case TileMode_e::TILE_VERTICAL:
                textureDef.samplerState.clampU = 1;
                textureDef.samplerState.clampV = 0;
                textureDef.samplerState.clampW = 0;
                break;
            ...
            case GdtFilter_e::MIP_2X_BILINEAR:
                textureDef.samplerState.filter = TEXTURE_FILTER_ANISO2X;
                textureDef.samplerState.mipMap = SAMPLER_MIPMAP_ENUM_NEAREST;
                break;
            case GdtFilter_e::MIP_2X_TRILINEAR:
                textureDef.samplerState.filter = TEXTURE_FILTER_ANISO2X;
                textureDef.samplerState.mipMap = SAMPLER_MIPMAP_ENUM_LINEAR;
                break;
            case GdtFilter_e::MIP_4X_BILINEAR:
                textureDef.samplerState.filter = TEXTURE_FILTER_ANISO4X;
                textureDef.samplerState.mipMap = SAMPLER_MIPMAP_ENUM_NEAREST;
                break;
            ...
            case GdtFilter_e::NOMIP_NEAREST:
                textureDef.samplerState.filter = TEXTURE_FILTER_NEAREST;
        ...
        void AddConstant(const std::string& constantName, const GdtVec4& literalData)
        {
            ...
            constantDef.literal.x = literalData.x;
            constantDef.literal.y = literalData.y;
            constantDef.literal.z = literalData.z;
            constantDef.literal.w = literalData.w;
        ...
        void SetAlphaTest(const AlphaTest_e alphaTest)
        {
            ...
            case AlphaTest_e::ALWAYS:
                m_base_state_bits.loadBits.structured.alphaTestDisabled = 1;
                break;

            case AlphaTest_e::GT0:
                m_base_state_bits.loadBits.structured.alphaTest = GFXS_ALPHA_TEST_GT_0;
                break;

            case AlphaTest_e::LT128:
                m_base_state_bits.loadBits.structured.alphaTest = GFXS_ALPHA_TEST_LT_128;
                break;

            case AlphaTest_e::GE128:
                m_base_state_bits.loadBits.structured.alphaTest = GFXS_ALPHA_TEST_GE_128;
                break;
        ...
        void SetBlendFunc(BlendOp_e blendOp, CustomBlendFunc_e srcFunc, CustomBlendFunc_e destFunc)
        {
            ...
            m_base_state_bits.loadBits.structured.blendOpRgb = static_cast<unsigned>(blendOp) - 1;
            m_base_state_bits.loadBits.structured.srcBlendRgb = static_cast<unsigned>(srcFunc) - 1;
            m_base_state_bits.loadBits.structured.dstBlendRgb = static_cast<unsigned>(destFunc) - 1;
        }
        ...
        void SetSeparateAlphaBlendFunc(BlendOp_e blendOp, CustomBlendFunc_e srcFunc, CustomBlendFunc_e destFunc)
        {
            ...
            m_base_state_bits.loadBits.structured.blendOpAlpha = static_cast<unsigned>(blendOp) - 1;
            m_base_state_bits.loadBits.structured.srcBlendAlpha = static_cast<unsigned>(srcFunc) - 1;
            m_base_state_bits.loadBits.structured.dstBlendAlpha = static_cast<unsigned>(destFunc) - 1;
        }
                           ...
                           const StateBitsEnabledStatus_e colorWriteAlpha)
        {
            ...
            m_base_state_bits.loadBits.structured.colorWriteRgb = 0;
            m_base_state_bits.loadBits.structured.colorWriteAlpha = 0;
            if (colorWriteRed == StateBitsEnabledStatus_e::ENABLED)
                m_base_state_bits.loadBits.structured.colorWriteRgb = 1;
            if (colorWriteAlpha == StateBitsEnabledStatus_e::ENABLED)
                m_base_state_bits.loadBits.structured.colorWriteAlpha = 1;
        }
        ...
        void SetCullFace(const CullFace_e cullFace)
        {
            ...
            {
                m_base_state_bits.loadBits.structured.cullFace = GFXS_CULL_FRONT;
            }
            else if (cullFace == CullFace_e::BACK)
            {
                m_base_state_bits.loadBits.structured.cullFace = GFXS_CULL_BACK;
            }
            else
            {
                assert(cullFace == CullFace_e::NONE);
                m_base_state_bits.loadBits.structured.cullFace = GFXS_CULL_NONE;
            }
        }
        ...
        void SetDepthTest(const DepthTest_e depthTest)
        {
            ...
            case DepthTest_e::LESS_EQUAL:
                m_base_state_bits.loadBits.structured.depthTestDisabled = 0;
                m_base_state_bits.loadBits.structured.depthTest = GFXS_DEPTHTEST_LESSEQUAL;
                break;

            case DepthTest_e::LESS:
                m_base_state_bits.loadBits.structured.depthTestDisabled = 0;
                m_base_state_bits.loadBits.structured.depthTest = GFXS_DEPTHTEST_LESS;
                break;

            case DepthTest_e::EQUAL:
                m_base_state_bits.loadBits.structured.depthTestDisabled = 0;
                m_base_state_bits.loadBits.structured.depthTest = GFXS_DEPTHTEST_EQUAL;
                break;

            case DepthTest_e::ALWAYS:
                m_base_state_bits.loadBits.structured.depthTestDisabled = 0;
                m_base_state_bits.loadBits.structured.depthTest = GFXS_DEPTHTEST_ALWAYS;
                break;

            case DepthTest_e::DISABLE:
                m_base_state_bits.loadBits.structured.depthTestDisabled = 1;
                m_base_state_bits.loadBits.structured.depthTest = 0;
                break;
        ...
        {
            if (depthWrite)
                m_base_state_bits.loadBits.structured.depthWrite = 1;
            else
                m_base_state_bits.loadBits.structured.depthWrite = 0;
        }

        void SetGammaWrite(const bool gammaWrite)
        {
            if (gammaWrite)
                m_base_state_bits.loadBits.structured.gammaWrite = 1;
            else
                m_base_state_bits.loadBits.structured.gammaWrite = 0;
        }
        ...
        void SetPolygonOffset(const PolygonOffset_e polygonOffset)
        {
            ...
            m_base_state_bits.loadBits.structured.polygonOffset = static_cast<unsigned>(polygonOffset) - 1;
        }
        ...
        void DisableStencil(const StencilIndex stencil)
        {
            ...
            m_base_state_bits.loadBits.raw[1] &= ~(enabledMask | funcMask | opFailMask | opZFailMask | opPassMask);
        }
            ...
            const StencilIndex stencil, StencilFunc_e stencilFunc, StencilOp_e stencilOpFail, StencilOp_e stencilOpZFail, StencilOp_e stencilOpPass)
        {
            ...
            m_base_state_bits.loadBits.raw[1] |= enabledMask;

            m_base_state_bits.loadBits.raw[1] &= ~funcMask;
            m_base_state_bits.loadBits.raw[1] |= ((static_cast<unsigned>(stencilFunc) - 1) << funcShift) & funcMask;

            m_base_state_bits.loadBits.raw[1] &= ~opFailMask;
            m_base_state_bits.loadBits.raw[1] |= ((static_cast<unsigned>(stencilOpFail) - 1) << opFailShift) & opFailMask;

            m_base_state_bits.loadBits.raw[1] &= ~opZFailMask;
            m_base_state_bits.loadBits.raw[1] |= ((static_cast<unsigned>(stencilOpZFail) - 1) << opZFailShift) & opZFailMask;

            m_base_state_bits.loadBits.raw[1] &= ~opPassMask;
            m_base_state_bits.loadBits.raw[1] |= ((static_cast<unsigned>(stencilOpPass) - 1) << opPassShift) & opPassMask;
        }
