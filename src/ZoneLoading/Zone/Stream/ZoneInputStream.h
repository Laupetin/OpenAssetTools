...
public:
    ...
    void InsertAlias(uintptr_t aliasValue, size_t offset) const;
...
public:
    ...
    virtual uintptr_t AllocLookupEntry(void** alias) = 0;

    template<typename T> uintptr_t AllocLookupEntry(T*& offset)
    {
        return AllocLookupEntry(reinterpret_cast<void**>(&offset));
    }
