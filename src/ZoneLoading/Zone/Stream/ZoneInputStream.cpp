...
void ZoneStreamFillReadAccessor::InsertAlias(const uintptr_t aliasValue, const size_t offset) const
{
...
namespace
{
    ...
    public:
        ...
        uintptr_t AllocLookupEntry(void** alias) override
        {
            // nullptr is always lookup alias 0
            if (*alias == nullptr)
                return 0;

            const auto newIndex = m_alias_lookup.size();

            m_alias_lookup.emplace_back(alias);
