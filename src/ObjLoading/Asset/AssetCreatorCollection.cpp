...
AssetCreationResult AssetCreatorCollection::CreateAsset(
    const asset_type_t assetType, const std::string& assetName, ISearchPath& searchPath, MemoryManager& memory, AssetCreationContext& context) const
{
    ...
    if (assetType >= 0 && static_cast<unsigned>(assetType) < m_asset_creators_by_type.size())
    {
        ...
        {
            const auto result = creator->CreateAsset(assetName, searchPath, memory, context);
...
AssetCreationResult AssetCreatorCollection::CreateDefaultAsset(const asset_type_t assetType,
                                                               const std::string& assetName,
                                                               MemoryManager& memory,
                                                               const AssetCreationContext& context) const
{
    ...
    {
        auto defaultAsset = m_default_asset_creators_by_type[assetType]->CreateDefaultAsset(assetName, memory);
        return AssetCreationResult::Success(context.AddAssetGeneric(defaultAsset));
    }
