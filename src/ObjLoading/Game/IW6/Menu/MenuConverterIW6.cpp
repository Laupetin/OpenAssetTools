#include "MenuConverterIW6.h"

#include "Menu/AbstractMenuConverter.h"
#include "MenuConversionZoneStateIW6.h"
...
namespace
{
    class MenuConverter final : public AbstractMenuConverter, public IMenuConverter
    {
                                          ...
                                          const int targetFunctionIndex) const
        {
            ...
            functionEntry.data.op = static_cast<operationEnum>(targetFunctionIndex);
                                                    ...
                                                    const CommonItemDef* item) const
        {
            if (!HandleSpecialBaseFunctionCall(gameStatement, entries, functionCall, menu, item))
            {
                expressionEntry functionEntry{};
                functionEntry.type = EET_OPERATOR;
                functionEntry.data.op = static_cast<operationEnum>(functionCall->m_function_index);
                entries.emplace_back(functionEntry);
        ...
        ItemFloatExpression*
            ConvertFloatExpressions(const CommonItemDef* commonItem, itemDef_t* item, const CommonMenuDef* parentMenu, int& floatExpressionCount) const
        {
        ...
        [[nodiscard]] listBoxDef_s* ConvertListBoxFeatures(itemDef_t* item,
        ...
        [[nodiscard]] editFieldDef_s* ConvertEditFieldFeatures(itemDef_t* item,
        ...
        [[nodiscard]] multiDef_s* ConvertMultiValueFeatures(itemDef_t* item,
        ...
        [[nodiscard]] newsTickerDef_s* ConvertNewsTickerFeatures(itemDef_t* item,
        ...
        [[nodiscard]] itemDef_t* ConvertItem(const CommonMenuDef& parentMenu, const CommonItemDef& commonItem) const
        {
            auto* item = m_memory.Alloc<itemDef_t>();
            ...
            item->type = static_cast<ItemDefType>(ConvertItemType(commonItem.m_type));
        ...
        itemDef_t** ConvertMenuItems(const CommonMenuDef& commonMenu, int& itemCount) const
        {
            ...
            auto* items = m_memory.Alloc<itemDef_t*>(commonMenu.m_items.size());
