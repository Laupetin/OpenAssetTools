    ...
    public:
        ...
        AssetCreationResult CreateAsset(const std::string& assetName, AssetCreationContext& context) override
        {
            ...
            while (csv.NextRow(currentRow))
            {
                ...
                // velocityMin
                if (!currentRow[10].empty())
                {
                    alias.velocityMin = std::stof(currentRow[10]);
                }
                else
                {
                    alias.velocityMin = 500000;
                }

                // channel
                std::string channelName = currentRow[11];
                ...
                {
                    alias.probability = std::stof(currentRow[13]);
                }
                else
                {
                ...
                // slavePercentage
                // TODO: Write flags
                if (!currentRow[15].empty())
                {
                    alias.slavePercentage = std::stof(currentRow[15].c_str());
                }
                else
                {
                ...
                {
                    alias.subtitle = m_memory.Dup(currentRow[17].c_str());
                }

                // Skip compression / index 18

                // secondaryAliasName
                if (!currentRow[19].empty() && currentRow[19] != "0")
                {
                    alias.secondaryAliasName = m_memory.Dup(currentRow[19].c_str());
                }

                // mixerGroup
                if (!currentRow[20].empty() && currentRow[20] != "0")
                {
                    alias.mixerGroup = m_memory.Dup(currentRow[20].c_str());
                }
                ...
                if (!currentRow[21].empty())
                {
                    ...
                    AssetRegistration<AssetSoundCurve> registration(currentRow[21], alias.volumeFalloffCurve);
                    context.AddAsset(std::move(registration));

                    alias.volumeFalloffCurve->filename = m_memory.Dup(currentRow[21].c_str());
                }

                if (!currentRow[22].empty())
                {
                    alias.startDelay = std::stoi(currentRow[22].c_str());
                }
                else
                {
                    alias.startDelay = 0;
                ...
                if (!currentRow[23].empty())
                {
                    ...
                    alias.speakerMap->name = m_memory.Dup(currentRow[23].c_str());
                }

                // TODO: Parse reverb into flags / index 24

                if (!currentRow[25].empty())
                {
                    alias.lfePercentage = std::stof(currentRow[25].c_str());
                }
                else
                {
                ...
                {
                    alias.centerPercentage = std::stof(currentRow[26].c_str());
                }
                else
                {
                ...
                // Skip compression / index 27

                if (!currentRow[28].empty())
                {
                    alias.envelopMin = std::stof(currentRow[28].c_str());
                }
                else
                {
                ...
                if (!currentRow[29].empty())
                {
                    alias.envelopMax = std::stof(currentRow[29].c_str());
                }
                else
                {
                ...
                if (!currentRow[30].empty())
                {
                    alias.envelopPercentage = std::stof(currentRow[30].c_str());
                }
                else
                {
