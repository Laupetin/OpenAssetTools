        ...
        bool LoadFile(ISearchPath& searchPath, std::string fileName)
        {
            ...
            while (std::getline(*file.m_stream, curline))
            {
                ...
                if ((outputIndicator = ((outputIndicator + 1) % outputEveryNth)) == 1)
                {
                    if (!meshname.empty())
                    {
                        std::cout << "\r- " << meshname << "\t| vertices > " << Positions.size() << "\t| texcoords > " << TCoords.size() << "\t| normals > "
        ...
        void VertexTriangluation(std::vector<unsigned int>& oIndices, const std::vector<Vertex>& iVerts)
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < int(tVerts.size()); i++)
                {
                    ...
                    for (int j = 0; j < int(iVerts.size()); j++)
                    {
                        if (algorithm::inTriangle(iVerts[j].Position, pPrev.Position, pCur.Position, pNext.Position) && iVerts[j].Position != pPrev.Position
