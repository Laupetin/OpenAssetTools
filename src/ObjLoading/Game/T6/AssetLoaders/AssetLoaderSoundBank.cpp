    ...
    const std::string& assetName, ISearchPath* searchPath, MemoryManager* memory, IAssetLoadingManager* manager, Zone* zone) const
{
    ...
    const auto aliasFile = searchPath->Open(std::format("soundbank/{}.aliases.csv", assetName));
    if (!aliasFile.IsOpen())
        return false;

    auto* sndBank = memory->Alloc<SndBank>();
    ...
    const auto radverbFile = searchPath->Open(std::format("soundbank/{}.reverbs.csv", assetName));
    ...
    const auto duckListFile = searchPath->Open(std::format("soundbank/{}.ducklist.csv", assetName));
