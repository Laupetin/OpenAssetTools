...
namespace
{
    ...
    public:
        ...
        AssetCreationResult CreateAsset(const std::string& assetName, AssetCreationContext& context) override
        {
            ...
            const auto loadResult = image::LoadIwi(ss);
            if (!loadResult)
            {
            ...
            if (texture->GetTextureType() == image::TextureType::T_2D)
                image->mapType = 3;
            else if (texture->GetTextureType() == image::TextureType::T_3D)
                image->mapType = 4;
            else if (texture->GetTextureType() == image::TextureType::T_CUBE)
                image->mapType = 5;
            else
                _ASSERT(false);
            ...
            if (image->noPicmip)
                image->texture.loadDef->flags |= image::iwi27::IMG_FLAG_NOMIPMAPS;

            if (texture->GetTextureType() == image::TextureType::T_3D)
                image->texture.loadDef->flags |= image::iwi27::IMG_FLAG_VOLMAP;

            if (texture->GetTextureType() == image::TextureType::T_CUBE)
                image->texture.loadDef->flags |= image::iwi27::IMG_FLAG_CUBEMAP;
...
namespace image
{
    std::unique_ptr<AssetCreator<AssetImage>> CreateLoadDefLoaderT6(MemoryManager& memory, ISearchPath& searchPath)
    {
