...
namespace
{
    ...
    {
    public:
        EfxParserT5(MemoryManager& memory, ISearchPath& searchPath)
            : m_memory(memory),
        ...
        AssetCreationResult ParseAndCreate(const std::string& assetName, AssetCreationContext& context)
        {
            ...
            fxDef->elemDefCountLooping = loopCount;
            fxDef->elemDefCountOneShot = oneShotCount;
            fxDef->elemDefCountEmission = emissionCount;
            ...
            if (totalElements > 0)
            {
                ...
                for (auto& elemData : elements)
                {
                    ...
                    elem.collMins[0] = elemData.collMins[0];
                    elem.collMins[1] = elemData.collMins[1];
                    elem.collMins[2] = elemData.collMins[2];
                    elem.collMaxs[0] = elemData.collMaxs[0];
                    elem.collMaxs[1] = elemData.collMaxs[1];
                    elem.collMaxs[2] = elemData.collMaxs[2];
                    ...
                    if (elemData.trailSplitDist != 0.0f || elemData.trailScrollTime != 0.0f || elemData.trailRepeatDist != 0.0f)
                    {
                        ...
                        if (!elemData.trailVerts.empty())
                        {
                            ...
                            {
                                trail->verts[v].pos[0] = elemData.trailVerts[v * 3 + 0];
                                trail->verts[v].pos[1] = elemData.trailVerts[v * 3 + 1];
                    ...
                    elem.billboardPivot[0] = elemData.billboardPivot[0];
                    elem.billboardPivot[1] = elemData.billboardPivot[1];
                }
            }
    ...
    private:
        void ParseElementBlock(std::istream& stream, ElementData& elem)
        {
            ...
            while (braceDepth > 0 && std::getline(stream, line))
            {
                ...
                else if (key == "collOffset")
                {
                    ...
                    elem.collMaxs[0] = ParseFloat(t1);
                    elem.collMaxs[1] = ParseFloat(t2);
                    elem.collMaxs[2] = ParseFloat(t3);
                }
        ...
        void ParseVelGraph(std::istream& stream, ElementData& elem, std::string_view key)
        {
            ...
            while (std::getline(stream, line))
            {
                ...
                auto tok1 = ReadToken(sv);
                auto tok2 = ReadToken(sv);
                basePoints.emplace_back(ParseFloat(tok1), ParseFloat(tok2));
            }
            ...
            while (std::getline(stream, line))
            {
                ...
                auto tok1 = ReadToken(sv);
                auto tok2 = ReadToken(sv);
                ampPoints.emplace_back(ParseFloat(tok1), ParseFloat(tok2));
            }
            ...
            for (size_t i = 0; i < basePoints.size() && i < sampleCount; i++)
                elem.velSamples[i].local.velocity.base[axis] = basePoints[i].second;
            for (size_t i = 0; i < ampPoints.size() && i < sampleCount; i++)
                elem.velSamples[i].local.velocity.amplitude[axis] = ampPoints[i].second;
        }
        ...
        void ParseTrailDefBlock(std::istream& stream, ElementData& elem)
        {
            ...
            while (std::getline(stream, line))
            {
                ...
                auto sv = std::string_view(line);
                elem.trailInds.push_back(static_cast<uint16_t>(ParseInt(ReadToken(sv))));
            }
        }
